...
export function PredictionSimulator({
  ...
  return (
    <div className="rounded-3xl border border-white/90 bg-white/80 backdrop-blur-xl p-6 sm:p-8 shadow-xl space-y-6">
      <div className="flex items-center justify-between border-b border-slate-200/80 pb-4">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-purple-100 text-purple-700 border border-purple-200 shadow-xs">
        ...
        <button
          ...
          className="text-xs font-mono font-semibold text-purple-700 hover:text-purple-900 flex items-center gap-1.5 border border-purple-200 px-3 py-1.5 rounded-xl bg-purple-50 transition shadow-xs"
        >
      ...
      {initialRealFlights && initialRealFlights.length > 0 && (
        <div className="space-y-2 rounded-2xl bg-slate-50/80 border border-slate-200/80 p-4 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-700 flex items-center gap-1.5">
              <Plane className="h-4 w-4 text-purple-600" />
              Real Scraped Flights from Database (Neon PostgreSQL):
            </span>
            <span className="text-[10px] font-mono font-bold text-purple-700 bg-purple-100 border border-purple-200 px-2.5 py-0.5 rounded-full">
              Click to Simulate
            </span>
          </div>
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 rounded-xl px-3 py-1.5 text-xs transition border ${
                    isSelected
                      ? "bg-purple-600 text-white border-purple-600 shadow-md font-semibold"
                ...
                >
                  ...
                  <span className={`font-mono text-[11px] ${isSelected ? "text-purple-100" : "text-slate-500"}`}>
      ...
      <div className="space-y-2">
        <label className="text-xs font-bold text-slate-600 uppercase tracking-wider flex items-center justify-between">
          ...
          <span className="text-[11px] font-mono text-purple-700 lowercase font-semibold">
            6 tracked corridors
          </span>
        </label>
          ...
          {corridorList.map((r) => {
            ...
            return (
              <button
                ...
                className={`rounded-xl px-3.5 py-2 text-xs font-mono transition border flex items-center gap-2 ${
                  isSelected
              ...
              >
                ...
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${isSelected ? "bg-purple-700 text-white" : "bg-slate-100 text-purple-700 border border-purple-200"}`}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs rounded-2xl bg-slate-100/80 border border-slate-200/80 p-4 font-mono">
        <div>
          <span className="text-[10px] text-slate-500 uppercase font-semibold block">Database Obs</span>
        ...
        <div>
          <span className="text-[10px] text-slate-500 uppercase font-semibold block">Live Avg Fare</span>
          <span className="font-bold text-purple-700 text-sm">
            ₹{selectedRoute.benchmark.toLocaleString("en-IN")}
          </span>
        </div>
        <div>
          <span className="text-[10px] text-slate-500 uppercase font-semibold block">Scraped Min / Max</span>
          <span className="text-slate-800 font-bold text-sm">
            ₹{selectedRoute.minFare?.toLocaleString("en-IN") || "1,000"} – ₹{selectedRoute.maxFare?.toLocaleString("en-IN") || "25,000"}
          </span>
        </div>
        <div>
          <span className="text-[10px] text-slate-500 uppercase font-semibold block">Route Base Value</span>
          <span className="text-slate-800 font-bold text-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-1">
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative flex items-center">
            ...
            <select
              ...
              className="w-full rounded-xl border border-slate-200 bg-white pl-9 pr-3 py-2.5 text-sm text-slate-900 font-medium focus:border-purple-600 focus:outline-none focus:ring-1 focus:ring-purple-600 shadow-xs"
            >
              {AIRLINES.map((a) => (
        ...
        {/* Stops */}
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-slate-700">Stops</label>
      ...
      {showFeatures && activeFeatures && (
        <div className="rounded-2xl bg-purple-50/80 border border-purple-200 p-4 space-y-3 animate-in fade-in duration-200">
          <div className="flex items-center justify-between">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2">
            {Object.entries(activeFeatures).map(([k, v]) => (
              <div key={k} className="rounded-xl bg-white border border-purple-200 p-2.5 shadow-xs">
